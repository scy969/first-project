...
3. ```echo 'text' >> file.txt``` - если необходимо добавить новую строку в файл;
* ```echo 'text' > file.txt``` - если необходимо добавить строку в файл и удалить остальные. 
...
* Команда ```git diff <hash1> <hash2>``` покажет изменения, произошедшие между коммитами. 
